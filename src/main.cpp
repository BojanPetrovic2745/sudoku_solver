						...
						else
						{
							...
							for (int i = 0; i < 9; i++)
							{
								...
								for (int j = 0; j < 9; j++)
								{
									...
									Mat cell = idealBox(col, row); //sudoku_ext(col, row)
#ifdef TEST_MAIN
							...
							if (mode == 5)
							{
								cvtColor(idealBox, idealBox, CV_GRAY2BGR); 
								...
								for (int i = 0; i < 9; i++)
								{
									...
									{
										int k = floor(((double)j) / 3);
										int ri1 = (i + 1)* cell_width + p * line_width;
										int ci1 = j * cell_height + k * line_width;
										...
										if (rec[i][j] != 0)
										{
											Point text_pos(ci1 + ci1 / 10, ri1 - ri1 / 10);
											stringstream ss;
											ss << (int)rec[i][j];
